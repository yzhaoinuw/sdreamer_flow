...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.trace_emdedding = nn.Parameter(torch.randn(1, n_traces, 1 , inner_dim))
    ...
    def forward(self, x, label):
        ...
        cls_eeg = repeat(self.cls_eeg, '() () n d -> b e n d', b=b, e=t)
        cls_emg = repeat(self.cls_emg, '() () n d -> b e n d', b=b, e=t)
        src_eeg = torch.cat([eeg, cls_eeg], dim=-2)
        src_emg = torch.cat([emg, cls_emg], dim=-2)
        trace = torch.cat([src_eeg, src_emg],dim=1)
        if self.mix_type == 2:
            trace = trace + self.trace_emdedding[:, :2*t]

        for block in self.transformer:
            trace = block(trace, context=None)
        
        cls = trace[:,:,-1]
        emb = rearrange(cls,'b t d -> b (t d)')
        # x_our --> [b, n, 2d]
        # emb = torch.cat(cls, dim=-1)
