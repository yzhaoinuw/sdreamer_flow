...
from layers.patchEncoder import LinearPatchEncoder, LinearPatchEncoder2, PatchEncoder
from layers.transformer import Transformer
from layers.norm import PreNorm
from layers.epochEncoder import mlp_proj, LSTM_Encoder
import librosa
import numpy as np
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.eeg_proj = PatchEncoder(patch_len,c_in, inner_dim)
        self.emg_proj = PatchEncoder(patch_len,c_in, inner_dim)

        self.eeg_encoder =  LSTM_Encoder(inner_dim, depth=e_layers, dropout=dropout,
                            activation=activation)
        self.emg_encoder =  LSTM_Encoder(inner_dim, depth=e_layers, dropout=dropout,
                            activation=activation)
