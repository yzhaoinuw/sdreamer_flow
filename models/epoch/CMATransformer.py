...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.patch_encs = nn.ModuleList(
            [LinearPatchEncoder2(trace_idx, patch_len, c_in, inner_dim) for trace_idx in range(n_traces)])
    
        self.eeg_transformer = nn.ModuleList(
            [MultiHeadAttention2(inner_dim, n_heads, d_head, dropout=dropout, activation=activation,
        ...
        self.eeg_cma = MultiHeadAttention2(inner_dim, n_heads, d_head, context_dim=inner_dim, dropout=dropout, activation=activation,
            norm=norm_type, mult=mult_ff)

        self.emg_cma = MultiHeadAttention2(inner_dim, n_heads, d_head, context_dim=inner_dim, dropout=dropout, activation=activation,
            norm=norm_type, mult=mult_ff)
