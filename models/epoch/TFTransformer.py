...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.eeg_transformer =  Transformer(
                                    ...
                                    cls = True, flag = "epoch", domain = "time",
        ...
        self.eeg_stft_transformer =  Transformer(
                                    patch_len, n_patches+1, e_layers, c_in, 
                                    inner_dim, n_heads = n_heads, d_head = d_head,
                                    dropout = dropout, path_drop = path_drop, 
                                    activation = activation, norm = norm_type, 
                                    mult = mult_ff, mix_type = args.mix_type, 
                                    cls = True, flag = "epoch", domain = "freq",
                                    output_attentions=self.output_attentions
                                )
        ...
        # self.emg_transformer = Transformer(patch_len, n_patches, e_layers, c_in, 
        #                                    inner_dim, n_heads = n_heads, dropout = dropout, 
        #                                    activation = activation, norm = norm_type,
        #                                    mult=mult_ff, mix_type = args.mix_type,
        #                                    cls = True, flag = "epoch")
        
        
        self.mlp_head = CLS_head(inner_dim, channels=4, c_out=c_out)
    ...
    def forward(self, x, label):
        ...
        eeg_freq, eeg_freq_attn= self.emg_stft_transformer(eeg_freq)
        emg_freq, emg_freq_attn = self.emg_stft_transformer(emg_freq)

        cls_eeg, cls_emg, cls_eeg_freq, cls_emg_freq = eeg[:,-1], emg[:,-1], eeg_freq[:,-1], emg_freq[:,-1]
        # x_our --> [b, n, 4d]
        emb = torch.cat([cls_eeg, cls_emg, cls_eeg_freq, cls_emg_freq], dim=-1)
        out = self.mlp_head(emb)
        out_dict = {
            "out": out,
            "eeg_attn": eeg_attn, 
            "emg_attn": emg_attn,
            "eeg_freq_attn": eeg_freq_attn,
            "emg_freq_attn": emg_freq_attn,
            "emb": emb, 
            "label": label
        }
