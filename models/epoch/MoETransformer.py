...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.mono_blocks =  nn.ModuleList(
            [MultiPathBlock(inner_dim, n_heads, d_head, dropout=dropout, activation=activation,
            norm=norm_type, mult=mult_ff, with_emffn=False) for _ in range(e_layers)])
        ...
        self.cross_blocks =  nn.ModuleList(
            [MultiPathBlock(inner_dim, n_heads, d_head, dropout=dropout, activation=activation,
            norm=norm_type, mult=mult_ff, with_emffn=True, n_patch= n_patches+1) for _ in range(e_layers)])
