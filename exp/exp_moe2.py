...
from utils.visualize import visualize_pred, visualize_pred_seq, visualize_tsne, visualize_attn, visualize_tsne_seq
...
class Exp_MoE(object):
    ...
    def _build_model(self):
        model_dict = {
            ...
            #'Macross': MacrossTransformer if self.args.data == "Epoch" else n2nMacrossTransformer,
    ...
    def _reload_model(self):
        ckpt = load_checkpoint(self.exp_dir, if_best=self.args.reload_best, device=self.device, reload_ckpt=self.args.reload_ckpt)
    ...
    def eval(self,val_loader, model, criterion, criterion2, criterion3, args):
        Time, Loss, Acc, F1, Acc_eeg, F1_eeg, Acc_emg, F1_emg, Kappa, Precision, Recall, \
            w_f1, s_f1, r_f1, w_prec, s_prec, r_prec, w_rec, s_rec, r_rec = build_tracker_mome(isEval=True)
        progress = ProgressMeter(
            len(val_loader),
            [Time, Loss, Acc, F1, Acc_eeg, F1_eeg, Acc_emg, F1_emg, Precision, Recall],
            prefix='Test: ' )
        all_pred = []
        all_gt = []
        all_pred_eeg, all_pred_emg = [], []
        self.model.eval()
        with torch.no_grad():
            end = time.time()
    ...
    def train(self, train_loader, model, criterion, criterion2, criterion3, optimizer, scheduler, epoch, device, args):
        
        Time, Loss, Acc, F1, Acc_eeg, F1_eeg, Acc_emg, F1_emg, Kappa, Precision, Recall = build_tracker_mome(isEval=False)
        ...
        for i, (traces, labels) in enumerate(train_loader):
            ...
            # loss_eeg = criterion(out_eeg, label.view(-1))
            # loss_emg = criterion(out_emg, label.view(-1))
            distill_eeg = criterion3(F.log_softmax(out_eeg/2.0, dim=1), F.softmax(out/2.0, dim=1))
            distill_emg = criterion3(F.log_softmax(out_emg, dim=1), F.softmax(out, dim=1))
            # distill_emg = 0.0
            loss = loss1 + (distill_eeg + distill_emg) * self.scale
            # loss = loss1
    ...
    def run_train(self, setting):
        ...
        scheduler = load_scheduler(self.args, optimizer=optimizer, train_steps=train_steps)

        for epoch in range(self.args.epochs):

            self.train(train_loader, self.model, criterion, criterion2, criterion3, optimizer, scheduler, epoch, self.device, self.args)
            print("\n")

            acc = self.eval(val_loader, self.model, criterion, criterion2, criterion3, self.args)
            print("\n")

            early_stopping(args=self.args, epoch=epoch, acc=acc, model=self.model,
                            optimizer=optimizer, exp_dir=self.exp_dir)
    ...
    def run_eval_visualize(self, setting):
        ...
        visual_model = self._reload_model()
        if 'pred' in self.args.visualize_mode:
            visualize_pred_seq(setting, visual_model, visualize_loader, self.device, self.args)
        # if 'attn' in self.args.visualize_mode:
        #     visualize_attn(setting, visual_model, visualize_loader, self.device, self.args)
        if 'emb' in self.args.visualize_mode:
            visualize_tsne_seq(setting, visual_model, visualize_loader, self.device, self.args)
