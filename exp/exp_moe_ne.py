...
SimMoE, FreqCM, NewMoE, NewMoE2, sDREAMERNE
...
from utils.metric_tracker import batch_updater, build_tracker_mome, build_tracker_mome_wNE
...
class Exp_MoE(object):
    ...
    def eval(self,val_loader, model, criterion, criterion2, criterion3, args):
        Time, Loss, Acc, F1, Acc_eeg, F1_eeg, Acc_emg, F1_emg, Acc_ne, F1_ne, \
        Kappa, Precision, Recall, \
        w_f1, s_f1, r_f1, w_prec, s_prec, r_prec, w_rec, s_rec, r_rec = build_tracker_mome_wNE(isEval=True)
    ...
    def train(self, train_loader, model, criterion, criterion2, criterion3, optimizer, scheduler, epoch, device, args):
        ...
        Time, Loss, Acc, F1, Acc_eeg, F1_eeg, Acc_emg, F1_emg, Acc_ne, F1_ne, \
        Kappa, Precision, Recall = build_tracker_mome_wNE(isEval=False)
        ...
        for i, (traces, nes, labels) in enumerate(train_loader):
            ...
            # loss_eeg = criterion(out_eeg, label.view(-1))
            # loss_emg = criterion(out_emg, label.view(-1))
            distill_eeg = criterion3(F.log_softmax(out_eeg, dim=1), F.softmax(out, dim=1))
