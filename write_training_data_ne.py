...
def prepare_data(mat_file, seq_len=512, augment=False, upsampling_scale=10):
...
def prepare_data_ne(mat_file, seq_len=512, augment=False, upsampling_scale=10):
    mat = loadmat(mat_file)
    eeg, emg, ne, sleep_scores = reshape_sleep_data_ne(mat)
    sleep_scores_len = len(sleep_scores)
    ne_len = len(ne)
...
if __name__ == "__main__":
    seq_len = 64  # epochs per sequence; Jaysen found 64 > 256/512
    fold = 1  # don't change
    # NE-bearing .mat recordings (must contain eeg/emg/ne/ne_frequency/sleep_scores).
    # Jaysen's groundtruth_data has the NE channel; relocate/copy locally as desired.
    data_path = "/gpfs/fs3/archive/dkell12_lab/BrainFlowZZZ/jaysenSS/sdreamer_train_jsc/groundtruth_data"
    save_path = f"../sdreamer_data_ne/n_seq_{seq_len}/fold_{fold}"  # where to save the train/val NE tensors
