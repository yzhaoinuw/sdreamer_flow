...
class MLP_Block(nn.Module):
    def __init__(self, dim, dropout=0.,
        ...
        if activation == "glu":
            ...
            relu_squared = True
        self.ff = FeedForward(dim, mult=1, dropout=dropout, glu=glu, relu=relu, relu_squared=relu_squared)
...
class MLP_Encoder(nn.Module):
    def __init__(self, dim, depth=3, dropout=0.,
            activation="glu"):
        super().__init__()
        self.mlp_blocks = nn.ModuleList([])
        for _ in range(depth):
            self.mlp_blocks.append(MLP_Block(dim, dropout=dropout, activation=activation))
...
class MLP_EpochEncoder(nn.Module):
    def __init__(self, seq_len, dim, depth=3, dropout=0.,
            activation="glu"):
        super().__init__()
...
class LSTM_Encoder(nn.Module):
    def __init__(self, dim, depth=3, dropout=0.,
            activation="glu"):
        super().__init__()
        self.bilstm = nn.LSTM(dim, dim, depth, dropout=dropout, batch_first=True, 
                            bidirectional=True)
...
class LSTM_EpochEncoder(nn.Module):
    def __init__(self, patch_len, dim, depth=3, dropout=0.,
            activation="glu"):
        super().__init__()
