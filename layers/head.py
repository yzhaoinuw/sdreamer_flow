...
class Pooler(nn.Module):
    def __init__(self, hidden_size, useRaw=False):
        ...
        self.pool_head = nn.Sequential(
            ...
            nn.Tanh()
            ) if not useRaw else nn.Identity()
        self.pool_head.apply(transformer.init_weights)
    def forward(self, hidden_states):
        last_token_tensor = hidden_states[:, 0] if hidden_states.ndim==3 else hidden_states[:,:,0]
...
class SeqPooler2(nn.Module):
    def __init__(self, hidden_size, useRaw=False):
        ...
        self.pool_head.apply(transformer.init_weights)
    def forward(self, hidden_states):
        hidden_states = torch.cat((hidden_states[:,: hidden_states.shape[1] // 2], hidden_states[:, hidden_states.shape[1] // 2:]), dim=-1)
        pooled_output = self.pool_head(hidden_states)
        return pooled_output