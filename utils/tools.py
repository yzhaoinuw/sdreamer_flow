...
def load_checkpoint(exp_dir, if_best, device, filename=None, defualt='ckpt.pth.tar', reload_ckpt=None):
    if filename is None:
    ...
    if os.path.isfile(reload_ckpt):
        ...
        print("=> loaded checkpoint '{}' (epoch {} bestAcc {}))"
              .format(reload_ckpt, checkpoint['epoch'], checkpoint['best_acc']))
        return checkpoint
    else:
    ...
    def __call__(self, args, epoch, acc, model, optimizer, exp_dir):
        ...
        save_checkpoint({
                ...
                'state_dict': model.state_dict() 
                    if not isinstance(model, torch.nn.DataParallel) else model.module.state_dict(),
                'best_acc': self.best_acc,
                'optimizer' : optimizer.state_dict(),
            }, 
            is_best, exp_dir, filename='ckpt.pth.tar'
        )
