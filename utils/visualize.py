...
def visualize_pred(setting, model, val_loader, device, args):
        ...
        patches = [ mpatches.Patch(color=color,alpha=0.25, label="{:s}".format(class_mapping[cls_idx]) ) for cls_idx, color in color_mapping.items() ]
        model.eval()
        with torch.no_grad():
            for i, (traces_no_norm, traces, labels, stamp) in enumerate(val_loader):
                ...
                val_idx = stamp[0]
                timestamp = stamp[1]
                x = np.linspace(timestamp, timestamp + traces.size(0), traces.size(0) * traces.size(-1))
                ...
                for j in range(traces.size(0)):
                    ax1.axvspan(x[j * traces.size(-1)], x[(j + 1) * traces.size(-1) -1], facecolor=color_mapping[labels[j].item()], alpha=0.25)
                # ax1.legend(loc='upper right')

                ax2 = plt.subplot(412, sharex=ax1)
                ax2.plot(x, EEG, label='EEG Prediction', linewidth=0.25, color='k')
                for j in range(traces.size(0)):
                    ax2.axvspan(x[j * traces.size(-1)], x[(j + 1) * traces.size(-1) -1], facecolor=color_mapping[preds[j].item()], alpha=0.25)
                ...
                for j in range(traces.size(0)):
                    ax3.axvspan(x[j * traces.size(-1)], x[(j + 1) * traces.size(-1) -1], facecolor=color_mapping[labels[j].item()], alpha=0.25)
                # ax3.legend(loc='upper right')

                ax4 = plt.subplot(414, sharex=ax1)
                ax4.plot(x, EMG, label='EMG Prediction', linewidth=0.25, color='k')
                for j in range(traces.size(0)):
                    ax4.axvspan(x[j * traces.size(-1)], x[(j + 1) * traces.size(-1) -1], facecolor=color_mapping[preds[j].item()], alpha=0.25)
                # ax4.legend(loc='upper right')
                
                fig.suptitle('idx: {}, time: {}'.format(val_idx.item(), timestamp.item()), size=20, y=0.95)
                fig.legend(handles=patches, loc='upper right', labelspacing=0.1, fontsize=12)
                fig.savefig('{}/idx: {}, time: {}'.format(fig_dir, val_idx.item(), timestamp.item()), dpi=400)
                plt.close(fig)
...
def visualize_tsne(setting, model, val_loader, device, args):
    ...
    with torch.no_grad():
        ...
        plt.scatter(embs_2d[:,0], embs_2d[:,1], lw=0, s=40, c=cmap[labels.astype(np.int)+1])
        # plt.xlim(-25, 25)
        # plt.ylim(-25, 25)
        # plt.axis('off')
        plt.axis('tight')
...
def visualize_attn(setting, model, val_loader, device, args):
    ...
    with torch.no_grad():
        for i, (_, traces, labels, stamp) in enumerate(val_loader):
            ...
            timestamp = stamp[1]
            out_dict = model(traces, labels)
            sub_dir = os.path.join(attn_dir, 'idx{}'.format(val_idx.item()),'time{}'.format(timestamp.item()))
            ...
            if 'eeg_attn' in out_dict.keys() and 'emg_attn' in out_dict.keys():
                ...
                for j in range(eeg_cls_att.shape[0]):
                    ...
                    fig.suptitle('Attention Map of EEG and EMG for idx: {}, time: {}-{}'.format(val_idx.item(), timestamp.item(), j), size=20, y=0.95)
                    fig.savefig('{}/sample{}_heatmap_eeg_emg.png'.format(sub_dir, j), dpi=400)
                    plt.close(fig)
                    ...
                    fine_eeg_att = (fine_eeg_att-fine_eeg_att.min())/(fine_eeg_att.max()-fine_eeg_att.min())
                    fine_emg_att = (fine_emg_att-fine_emg_att.min())/(fine_emg_att.max()-fine_emg_att.min())
                    ...
                    fig.suptitle('Attention Map of EEG and EMG for idx: {}, time: {}-{}'.format(val_idx.item(), timestamp.item(), j), size=20, y=0.95)
                    fig.savefig('{}/sample{}_fine_eeg_emg.png'.format(sub_dir, j), dpi=400)
                    plt.close(fig)
            ...
            if 'cm_eeg_attn' in out_dict.keys() and 'cm_emg_attn' in out_dict.keys():
                ...
                for j in range(cm_eeg_cls_att.shape[0]):
                    ...
                    sns.heatmap(rough_eeg_att, ax=ax1)
                    sns.heatmap(rough_emg_att, ax=ax2)
                    fig.suptitle('Cross Attention Map of EEG and EMG for idx: {}, time: {}-{}'.format(val_idx.item(), timestamp.item(), j), size=20, y=0.95)
                    fig.savefig('{}/sample{}_heatmap_eeg_emg_cm.png'.format(sub_dir, j), dpi=400)
                    plt.close(fig)
                    ...
                    fig.suptitle('Cross Attention Map of EEG and EMG for idx: {}, time: {}-{}'.format(val_idx.item(), timestamp.item(), j), size=20, y=0.95)
                    fig.savefig('{}/sample{}_fine_eeg_emg_cm.png'.format(sub_dir, j), dpi=400)
                    plt.close(fig)
...
def visualize_pred_seq(setting, model, val_loader, device, args):
        ...
        patches = [ mpatches.Patch(color=color,alpha=1.0, label="{:s}".format(class_mapping[cls_idx]) ) for cls_idx, color in color_mapping2.items() ]
        model.eval()
        with torch.no_grad():
            for i, (traces_no_norm, traces, labels, stamp) in enumerate(val_loader):
                #TODO: split this data loader!
                ...
                for j in range(traces.size(0)):
                    ax1.axvspan(x[j * traces.size(-1)], x[(j + 1) * traces.size(-1) -1], facecolor=color_mapping[labels[j].item()], alpha=alpha)
                # ax1.legend(loc='upper right', fontsize=22)

                ax2 = plt.subplot(412, sharex=ax1)
                ax2.plot(x, EEG, label='EMG Prediction', linewidth=0.25, color='k')
                for j in range(traces.size(0)):
                    ax2.axvspan(x[j * traces.size(-1)], x[(j + 1) * traces.size(-1) -1], facecolor=color_mapping[preds[j].item()], alpha=alpha)
                ...
                for j in range(traces.size(0)):
                    ax3.axvspan(x[j * traces.size(-1)], x[(j + 1) * traces.size(-1) -1], facecolor=color_mapping[labels[j].item()], alpha=alpha)
                # ax3.legend(loc='upper right', fontsize=22)

                ax4 = plt.subplot(414, sharex=ax1)
                ax4.plot(x, EMG, label='EEG Prediction', linewidth=0.25, color='k')
                for j in range(traces.size(0)):
                    ax4.axvspan(x[j * traces.size(-1)], x[(j + 1) * traces.size(-1) -1], facecolor=color_mapping[preds[j].item()], alpha=alpha)
                # ax4.legend(loc='upper right',  fontsize=22)
                
                # fig.suptitle('idx: {}, time: {}'.format(val_idx.item(), timestamp.item()), size=20, y=0.95)
                fig.legend(handles=patches, loc='upper right', labelspacing=0.1, fontsize=28)
                fig.savefig('{}/idx: {}, time: {}'.format(fig_dir, val_idx.item(), timestamp.item()), dpi=400)
                plt.close(fig)
...
def visualize_tsne_seq(setting, model, val_loader, device, args):
    ...
    with torch.no_grad():
        ...
        for i, embs in enumerate([embs, eeg_embs, emg_embs]):
            ...
            plt.scatter(embs_2d[:,0], embs_2d[:,1], lw=0, s=40, c=cmap[labels.astype(np.int64)])
            # plt.xlim(-25, 25)
            # plt.ylim(-25, 25)
            plt.axis('off')
            plt.axis('tight')
